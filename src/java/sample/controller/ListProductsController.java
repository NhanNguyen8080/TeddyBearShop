...
public class ListProductsController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            String bearID = (String) request.getAttribute("BEAR_ID");
            List<TeddyBearDTO> list = new ArrayList<>();
            TeddyBearDAO dao = new TeddyBearDAO();
            if (bearID == null) {
                list = dao.getBearList();
            } else {
                if (bearID.trim().length() > 0) {
                    list = dao.getBearList(bearID);
                } else {
                    list = null;
                }
            }
            url = PRODUCTS_PAGE;
            session.setAttribute("BEAR_LIST", list);
            if (list == null) {
                request.setAttribute("NO_RESULT_SEARCH", "No results were found");
            }
